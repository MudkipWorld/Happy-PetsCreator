...
func recolor_part():
	...
	if selected_data is not Attirbute.CorgiAttirbutes: return
	...
	%Bib.self_modulate = selected_data.color_layers['bib']
	%Muzzle.self_modulate = selected_data.color_layers['muzzle']
	%Muzzle2.self_modulate = selected_data.color_layers['muzzle']
